...
armsManager = {
    ...
    init:function(){
        ...
        armsManager.loginInterface=new LoginInterface();  
        ...
        armsManager.armsMainMenuPanel= new Ext.Panel({
            ...
            bodyStyle : {
                background: armsManager.bodyColor
                },
            anchor:'100% 100%',
            margins:'5 0 5 5',
            layout:'accordion',
        ...
        /* var firebugWarning = function () {
                        ...
                        hideMask.defer(250);*/
        armsManager.firebugWarning = function () {
            var cp = new Ext.state.CookieProvider();

            if(window.console && window.console.firebug && ! cp.get('hideFBWarning')){
                Ext.Msg.show({
                    title:'Firebug Warning',
                    msg: 'Firebug is known to cause performance issues with the Archiving Server Manager Application.',
                    buttons: Ext.Msg.OK,
                    icon: Ext.MessageBox.WARNING
                });
            }
        ...
        armsManager.hideMask = function () {
            Ext.get('loginlogo').remove();
            Ext.get('loginfields').remove();
            Ext.fly('loading-mask').fadeOut({
                remove:true,
                callback : armsManager.firebugWarning
            });
