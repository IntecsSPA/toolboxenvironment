...
public class Toolbox extends AxisServlet implements ServletContextListener {
    ...
    public Document executeServiceRequest(String operationName, Document soapRequestDoc, String requestURI, boolean debugMode) throws IOException, ToolboxException {
        ...
        Hashtable<String, Hashtable<String, String>> serviceVariables=service.getImplementedInterface().getUserVariable();
        
        Hashtable<String, String> remoteSoapActionAttributes= new Hashtable<String, String>();
        remoteSoapActionAttributes.put("value", operationName);
        remoteSoapActionAttributes.put("type", "string");
        remoteSoapActionAttributes.put("displayedText", "SOAP action");
        serviceVariables.put("remoteOperation",  remoteSoapActionAttributes);
...
//            responseDocument = executeServiceRequest(operationName, soapRequestDocument, requestURI.substring(0,requestURI.lastIndexOf("/")), false);
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            tbxConfig.setApplicationName(config.getInitParameter("application"));
    ...
    public void processConfiguration() throws Exception {
       ...
       // logger = LogResourcesPersistence.getInstance().getRollingLogForToolbox();
        ...
        logger.info(tbxConfig.getApplicationName()+" START");
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, java.io.IOException {
        ...
        try {
            ...
            applicationName=requestURI.split("/")[1];
            logger.debug("Received request " + requestURI);

            webAppStart = applicationName+"/services/";
            if (requestURI.startsWith("/"+applicationName+"/rest")) {
                executeRestCommand(req, resp);
            }

            if (requestURI.startsWith("/"+applicationName+"/http")) {
            ...
            if (id != null && id.equals("") == false && service != null && service.equals("CSW-ebRIM") && request != null && request.equals("GetRepositoryItem")) {
                ...
                } else if (requestURI.startsWith("/"+applicationName+"/manager")) {
...
//            super.doPost(req, resp);
        ...
        if (requestURI.startsWith("/"+applicationName+"/debug")) {
            debugServiceRequest(resp, req, requestURI);
        } else if (requestURI.startsWith("/"+applicationName+"/deploy")) {
            try {
            ...
            executeManagerCommands(req, resp);
        } else if (requestURI.startsWith("/"+applicationName+"/rest")) {
            executeRestCommand(req, resp);
        }
