...
<xsl:stylesheet version="1.0" xmlns:eoli="http://earth.esa.int/XML/eoli" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:csw="http://www.opengis.net/cat/csw" xmlns:hma="http://earth.esa.int/hma" xmlns:ows="http://www.opengeospatial.net/ows" xmlns:gml="http://www.opengis.net/gml" xmlns:ogc="http://www.opengis.net/ogc" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template match="/">
		<xsl:variable name="position">
			<xsl:choose>
				<xsl:when test="$startPosition + $maxRecords - 1 &lt; count(eoli:response/eoli:retrievedData/eoli:Metadata)">
					<xsl:value-of select="$startPosition + $maxRecords - 1"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="count(eoli:response/eoli:retrievedData/eoli:Metadata)"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
			...
			<xsl:otherwise>
				<csw:GetRecordsResponse version="2.0.1">
					...
					<csw:SearchResults elementSet="summary" recordSchema="http://earth.esa.int/hma" resultSetId="resultSetId">
						<xsl:attribute name="numberOfRecordsMatched"><xsl:value-of select="count(eoli:response/eoli:retrievedData/eoli:Metadata)"/></xsl:attribute>
						<xsl:attribute name="numberOfRecordsReturned"><xsl:value-of select="$resultsReturned"/></xsl:attribute>
						<xsl:if test="$position &lt; count(eoli:response/eoli:retrievedData/eoli:Metadata)">
	...
	<xsl:template match="eoli:response/eoli:retrievedData/eoli:Metadata">
		<xsl:if test="(position() &lt;= $startPosition + $maxRecords - 1) and (position() &gt;= $startPosition)">
			<hma:EarthObservationProduct gml:id="DS_2044263010822">
				<xsl:attribute name="gml:id"><xsl:value-of select="eoli:dataIdInfo/eoli:idCitation/eoli:resTitle"/></xsl:attribute>
				<hma:identifier><xsl:value-of select="$collection"/></hma:identifier>
				<hma:parentIdentifier>urn:HMA:EoliCatalogue:<xsl:value-of select="substring-before(substring-after($collection,'urn:HMA:EoliCatalogue:'),':')"/>
							...
							<gml:exterior>
								<gml:LinearRing>
									<gml:posList>
										...
										<xsl:value-of select="translate(eoli:dataIdInfo/eoli:dataExt/eoli:geoEle/eoli:polygon/eoli:coordinates,'A,B', 'a b')"/>
										<xsl:value-of select="string(' ')"/>
										<xsl:value-of select="translate(substring-before(eoli:dataIdInfo/eoli:dataExt/eoli:geoEle/eoli:polygon/eoli:coordinates,' '),'A,B', 'a b')"/>
									</gml:posList>
								</gml:LinearRing>
							</gml:exterior>
						</gml:Polygon>
						...
						<hma:platform>
							<hma:Platform>
								...
								<hma:serialIdentifier>
									<xsl:value-of select="eoli:dataIdInfo/eoli:plaInsId/eoli:platfSer"/>
								</hma:serialIdentifier>
							</hma:Platform>
						</hma:platform>
						<hma:instrument>
				...
				<xsl:if test="eoli:dqInfo/eoli:graphOver/eoli:bgFileName != '' ">
					<hma:browse>
						<hma:BrowseInformation>
							<hma:type>QUICKLOOK</hma:type>
							<hma:referenceSystemIdentifier codeSpace="EPSG"/>
							<hma:fileName>
								<xsl:value-of select="eoli:dqInfo/eoli:graphOver/eoli:bgFileName"/>
							</hma:fileName>
						</hma:BrowseInformation>
					</hma:browse>
				</xsl:if>
