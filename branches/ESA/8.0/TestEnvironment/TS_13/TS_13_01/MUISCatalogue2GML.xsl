...
<xsl:stylesheet version="1.0" xmlns="http://www.esa.int/xml/schemas/mass/serviceresult" xmlns:mass="http://www.esa.int/mass" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:oi="http://www.esa.int/oi" xmlns:aoi="http://www.gim.be/xml/schemas/aoifeatures" xmlns:eoli="http://earth.esa.int/XML/eoli" xmlns:gml="http://www.opengis.net/gml">
	<xsl:param name="hostName"/>
	<xsl:template match="/">
		<xsl:apply-templates select="eoli:response"/>
	</xsl:template>
	<xsl:template match="eoli:response">
		<featureCollection xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.esa.int/xml/schemas/mass/serviceresult http://services.eoportal.org/schemas/1.0/serviceresult.xsd">
			<gml:boundedBy>
	...
	<xsl:template match="eoli:retrievedData">
		<xsl:for-each select="eoli:Metadata">
			<gml:featureMember>
				<Feature>
<!--					<xsl:attribute name="fid">srFeature.<xsl:number value="position()"/></xsl:attribute>-->
					<id><xsl:value-of select="eoli:dataIdInfo/eoli:idCitation/eoli:resTitle"/></id>
					<tooltip>MUIS:<xsl:value-of select="eoli:dataIdInfo/eoli:idCitation/eoli:resTitle"/></tooltip>
