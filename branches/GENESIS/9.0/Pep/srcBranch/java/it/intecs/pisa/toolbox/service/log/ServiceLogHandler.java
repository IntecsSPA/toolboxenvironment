...
public class ServiceLogHandler {
    public static InputStream extractLog(String service,int index) throws Exception {
        ...
        return getHandler(pages,logFile,service);
    }
    ...
    private static InputStream getHandler(int pageNumber, File logFile, String service)throws Exception{
        ...
        if(logType.equals("json"))
            return Util.addHeaderAndFooter("{ \"totalFileNumber\" : \"" + pageNumber + "\", \"service\" : \"" + service + "\", \"logs\" : [ {\"level\":\"\", \"thread\":\"\", \"date\":\"\", \"text\":\"LOG\" }", new FileInputStream(logFile), "]}");
        else
            return Util.addHeaderAndFooter("<?xml version=\"1.0\" encoding=\"UTF-8\"?><logs xmlns=\"http://pisa.intecs.it/mass/toolbox/log\" serviceName=\"" + service + "\"  pagesNumber=\"" + pageNumber + "\">", new FileInputStream(logFile), "</logs>");
    }
}
