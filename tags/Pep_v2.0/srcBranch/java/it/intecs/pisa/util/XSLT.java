...
public class XSLT {
  ...
  public static void addParameter(Document xsl, String parameterName, String parameterValue) {
    ...
    LinkedList param=DOMUtil.getChildrenByTagName(rootElement,"xsl:param");
    for(int i=0; i<param.size();i++){
        if(((Element)param.get(i)).getAttribute("name").equals(parameterName)){
           ((Element)param.get(i)).setTextContent(parameterValue);
           return;
        }
    }
