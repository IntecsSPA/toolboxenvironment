	...
	<xsl:template match="@*" xml:space="preserve">
		...
		<SPAN class="att">
				...
				<xsl:when test="name()='schemaLocation'">
                                    ...
                                    <!-- a href="#" ONCLICK="javascript:parent.schemaviewxmlwin{string-length(.)}=parent.dhtmlwindow.open('schemaviewxml{string-length(.)}', 'iframe', 'displayXML.jsp?outputType=XML&amp;resourceKey={.}', '{.}', 'width=830px,height=720px,left=5px,top=5px,resize=1,scrolling=1'); return false">
										<xsl:value-of select="."/>
									</a -->
                                    <a href="#" onclick="javascript:parent.openTab('xml','Tab', 'displayXML.jsp?outputType=XML&amp;resourceKey={.}', '{.}');">
										<xsl:value-of select="."/>
									</a>
                                                                    </xsl:when>
				<xsl:otherwise>
