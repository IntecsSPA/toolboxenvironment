      ...
      <xsl:for-each select="child::*">
        ...
        <xsl:choose>
          <xsl:when test="$maxOccursInput &gt; 1">
            ...
            <if>
              <mkdir>
                <string><xsl:value-of select="$multipleInputPath"/></string>
              </mkdir>
              <sequence>
                      ...
                      <xsl:choose>
                        ...
                        <xsl:otherwise>
                          <setVariable name="localURI">
                            <string><xsl:value-of select="$multipleInputPath"/><xsl:value-of select="position()"/>
                            </string>
                          </setVariable>
                          <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <xsl:choose>
              <xsl:when
								test="(($remoteProtocol = 'ftp') or ($remoteProtocol = 'FTP'))">
								<!-- 
								...
								<xsl:variable name="user">
									<xsl:if
										test="$credentials">
										<xsl:value-of
											select="substring-before($credentials, ':')" />
									</xsl:if>
								</xsl:variable>
								<xsl:variable name="password">
									<xsl:if
										test="$credentials">
										<xsl:value-of
											select="substring-after($credentials, ':')" />
									</xsl:if>
								</xsl:variable>
								...
								<xsl:variable name="tmpAddress">
									<xsl:choose>
										<xsl:when
											...
											<xsl:value-of
												select="substring-after($address, '@')" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of
												select="$address" />
								...
								<xsl:variable name="tmpHost"
									select="substring-before($tmpAddress, '/')" />
								<xsl:variable name="ftpHost">
									<xsl:choose>
										<xsl:when
											test="contains($tmpHost, ':')">
											<xsl:value-of
												select="substring-before($tmpHost, ':')" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of
												select="$tmpHost" />
								...
								<xsl:variable name="ftpPort">
									<xsl:choose>
										<xsl:when
											test="contains($tmpHost, ':')">
											<xsl:value-of
												select="substring-after($tmpHost, ':')" />
										</xsl:when>
										<xsl:otherwise>
								...
								<xsl:variable name="ftpRemotePath"
									select="concat('/', substring-after($tmpAddress, '/'))" />
              ...
              <xsl:otherwise>
                <dumpFile fileType="binary">
                  ...
                  <http fileType="binary" method="GET">
                    <url>
                      <string><xsl:value-of select="@xlink:href"/></string>
                    </url>
                  </http>
                </dumpFile>
              </xsl:otherwise>
